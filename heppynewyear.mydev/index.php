...
function secondToDate($mounth, $day)
{
    ...
    //var_dump($currentDateArray);exit;
//var_dump($currentDateArray); exit;
...
//    $dateFrom = date_create($currentDateArray[0]."-".$currentDateArray[1]."-".$currentDateArray[2]."-".$currentDateArray[3].":".$currentDateArray[4].":".$currentDateArray[5]);
    $dateFrom = date_create($currentDateArray[0] .
        "-" . $currentDateArray[1] . "-" . $currentDateArray[2] .
        " " . $currentDateArray[3] . ":" . $currentDateArray[4] .
        ":" . $currentDateArray[5]);
    //var_dump($currentDateArray); exit;

    $dateTo = date_create($year . "-" . $mounth . "-" . $day);

    //var_dump($dateFrom);exit;
    $diff = date_diff($dateTo, $dateFrom);

    return ($diff->y * 265 * 24 * 60 * 60) +
        ($diff->m * 30 * 24 * 60 * 60) +
        ($diff->d * 24 * 60 * 60) +
        ($diff->h * 60 * 60) +
        ($diff->i * 60) +
